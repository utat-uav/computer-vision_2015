...
int	SetEdgeSimple_Outlier( MYPOINT *Pnt, int nPnt, double **r_array, double E_Ave )
{
	...
	for( i=0; i<nPnt; i++ )
	{
		...
		for( j=i+1; j<nPnt; j++ )
		{
			...
			nEdge = (int)min( nEdge, nMaxPnt*1.2 );
		}
	}
...
double HungarianMatch( double **costmat, MYPOINT *RowPnt, int nRowPnt, MYPOINT *ColPnt, int nColPnt)
{
	...
	cost = HungarianMatch( costmat, RowPnt, nRowPnt, ColPnt, nColPnt, max(nRowPnt, nColPnt) );
	return cost;
}
...
int SetMinMatch( double **costmat, MYPOINT *PntRow, int nPntRow, MYPOINT *PntCol, int nPntCol, int nMinMatch, double nMinConf )
{
	...
	HungarianMatch( costmat, PntRow, nPntRow, PntCol, nPntCol, max(nPntRow, nPntCol) );
	...
	MATCH_PAIR	*MatchPair = new MATCH_PAIR[min(nPntRow, nPntCol)];
	int			nMatchPair = 0;
	for( i=0; i<nPntRow; i++ )
	{
...
CString GetFileName ( CString strFileName )
{
    ...
    if ( (pos=strFileName.ReverseFind('\\')) >= 0 )
        return strFileName.Right ( l-pos-1 ) ;
    else if ( (pos=strFileName.ReverseFind(':')) >= 0 )
        return strFileName.Right ( l-pos-1) ;
    else
        return strFileName;
}
...
CString GetFileNameWithoutExt ( CString strFileName )
{
    ...
    l = strFileName.GetLength() ;
    if ( (pos=strFileName.ReverseFind('\\')) >= 0 )
        csFileName = strFileName.Right ( l-pos-1 ) ;
    else if ( (pos=strFileName.ReverseFind(':')) >= 0 )
        csFileName = strFileName.Right ( l-pos-1) ;
    else
        csFileName = strFileName;

	pos = csFileName.ReverseFind( '.' );
	if( pos >= 0 )
		return csFileName.Left( pos );
	else
		return csFileName;
}
