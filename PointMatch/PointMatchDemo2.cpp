...
int SingleTest( char *fnPntModel, char *fnPntDeform, char *fnMatch )
{
	...
	int nMinPnt = min( nPntModel, nPntDeform );
	int nMaxPnt = max( nPntModel, nPntDeform );
...
//Parse command line
	...
	char	fnMatch[MAX_PATH];
	char	fnPntModel[MAX_PATH];
	char	fnPntDeform[MAX_PATH];
	strcpy( fnPntModel, argv[argc-2] );
	strcpy( fnPntDeform, argv[argc-1] );

	//Point matching
	sprintf( fnMatch, "%s_%s.match", GetFileNameWithoutExt( fnPntModel ), GetFileNameWithoutExt( fnPntDeform ) );
