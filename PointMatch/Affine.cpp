				  ...
				  MYPOINT &Trans, double A[2][2] )
{
	...
	int		*ValidPnt = new int[min(nPntModel, nPntDeform)];
	int		nValidPnt = 0;
	for( i=0; i<nPntModel; i++ )
	{
		if( PntModel[i].nMatch < 0 )
	...
	if( nValidPnt < 0.3*min( nPntModel, nPntDeform) ) //Too few matching pairs. Use them all to estimate the affine transformation
		AffineEst( PntModel, nPntModel, PntDeform, nPntDeform, Trans, A );
	else
	{
